...
const ProjectDetails = () => {
  const { id } = useParams();
  const [project, setProject] = useState<typeof projects[0] | null>(null); // Change the type here

  useEffect(() => {
    const foundProject = projects.find((p) => p.id === parseInt(id.toString()));
    if (foundProject) {
      setProject(foundProject); // Now this is valid
    }
  }, [id]);
  ...
  return (
    ...
    <div className="bg-cover bg-center text-center overflow-hidden"
        style={{"backgroundImage":`${project.imgSrc}`,minHeight:'500px'}}
